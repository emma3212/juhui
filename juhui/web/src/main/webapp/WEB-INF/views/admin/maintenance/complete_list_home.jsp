...
<tiles:insertDefinition name="admin">
    ...
    <tiles:putAttribute name="body" value="/WEB-INF/views/admin/maintenance/body/complete_list_body.jsp" />
...
<script type="text/template" id="tpl">
    ...
    <tr data-order-id="&{it.id}">
        ...
        <td data-id="&{it.id}">
            ...
            未评价
            {@else if it.estimateStatus == "1"}
            系统好评
            {@else if it.estimateStatus == "2"}
            客户好评
            {@else}
            <a href="javascript:;" class="J_badEvaluate error-tip">客户差评</a>
            {@/if}
        </td>
        <td>
...
<script type="text/template" id="badEvaluateTipTemp">
    <p>&{data.estimate}</p>
</script>
<script type="text/javascript">
    KISSY.ready(function(S){
        S.use('widget/calendar, widget/dropdown, page/order/maintainenceOrder/maintainenceOrderList',function(){
            PW.page.order.maintainenceOrder.maintainenceOrderList.Complete({
                pagi: {
                    renderTo: '#J_pagination',
                    juicerRender: '#tpl',
                    dataRender: '#J_template',
                    pageSize: 10,
                    url: '${website}admin/maintenance/order/finish/ajax/list',
