...
<tiles:insertDefinition name="vip">
    ...
    <tiles:putAttribute name="body" value="/WEB-INF/views/vip/maintenance/body/finish_list_body.jsp" />
...
<script type="text/template" id="tpl">
    ...
    <tr data-order-id="&{it.id}">
        ...
        <td>&{it.thruTime}</td>
        <td>&{it.consumingTime}</td>
        <td>&{it.type}</td>
        <td data-id="&{it.employeeId}">
            <a href="javascript:;" class="J_servicePersonInfo">&{it.employee}</a>
        </td>
        <td>
            {@if it.estimateStatus == "0"}
            <input type="hidden" class="J_estStatus" value="&{it.estimateStatus}" />
            <input type="hidden" class="J_estimate" value="&{it.estimate}" />
            <a href="javascript:;" class="J_incomplete">未评价</a>
            {@else}
            <input type="hidden" class="J_estStatus" value="&{it.estimateStatus}" />
            <input type="hidden" class="J_estimate" value="&{it.estimate}" />
            <a href="javascript:;" class="J_complete">已评价</a>
            {@/if}
        </td>
        <td>&{it.presentExp}</td>
        <td>
...
<script type="text/javascript">
    KISSY.ready(function(S){
        S.use('widget/calendar, widget/dropdown, page/freemaintainence/order-list',function(){
            PW.page.freemaintainence.orderList.Complete({
                pagi: {
                    renderTo: '#J_pagination',
                    juicerRender: '#tpl',
                    dataRender: '#J_template',
                    pageSize: 10,
                    url: '${website}user/maintenance/order/finish/ajax/list',
