...
public class ShiroAuthorizingRealm extends AuthenticatingRealm
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        try {
            group = userGroupService.queryUserGroupByLoginName(loginName);
        } catch (SSException e) {
        ...
        namesSet.add(group.getName());

        // 查询权限组列表
        List<UserPermission> permissions = Collections.emptyList();
        try {
            permissions = userPermissionService.queryPermissionByLoginName(loginName);
        } catch (SSException e) {
            LogClerk.errLog.error(e);
            // 由于这里不支持，因此抛出一个运行时异常
            throw new RuntimeException(e);
        ...
        for(UserPermission p: permissions) {
            permissionsSet.add(p.getExpression());
        }

        SimpleAuthorizationInfo authorizationInfo = new SimpleAuthorizationInfo();
        authorizationInfo.setRoles(namesSet);
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        User user = null;
        try {
            user = userService.queryUerByLoginName(userName);
        } catch (SSException e) {
            LogClerk.errLog.error(e);
            // 由于这里不支持，因此抛出一个运行时异常
            throw new RuntimeException(e);
        ...
        SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(
                ...
                user.getPwd(), //密码
                ByteSource.Util.bytes(Integer.valueOf(user.hashCode()).toString()),//salt=username+salt
                getName() //realm name
        );
    ...
    /**
     * Returns {@code true} if authorization caching should be utilized if a {@link org.apache.shiro.cache.CacheManager} has been
    ...
    /**
     * Sets whether or not authorization caching should be utilized if a {@link org.apache.shiro.cache.CacheManager} has been
     * {@link #setCacheManager(org.apache.shiro.cache.CacheManager) configured}, {@code false} otherwise.
     * <p/>
     * The default value is {@code true}.
     *
